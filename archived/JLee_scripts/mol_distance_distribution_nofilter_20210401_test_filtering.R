...
mol_distance_distribution_20210401_test_filtering<-function(rcmappath, xmap, qcmappath, startlabelid, endlabelid, nk=NULL, additional_noise_removal=FALSE){
...
if(nrow(completeframe)<10){   ###Too few molecules to be meaningful
  ...
  if(additional_noise_removal==TRUE){         ###Typically only needed if two clusters are close and have issue separating due to noise at further regions
    p1<-ggplot(data.frame(completeframeoriginal), aes(x=completeframeoriginal))+geom_histogram(aes(x=completeframeoriginal, y=..density..), fill=NA, color="black")
    pdc<-ggplot_build(p1)
    #pdc$data[[1]]
...
###Plotting function
    Plotting_hist_and_Gau<-function(mod){
      ...
      adjusteddnorm<-function(data, mm, sd, prop){
        re<-prop*dnorm(data, mean=mm, sd=sd)
        return(re)
      }
      
      p1<-ggplot(data.frame(completeframeoriginal), aes(x=completeframeoriginal))+geom_histogram(aes(x=completeframeoriginal, y=..density..), fill=NA, color="black", binwidth=200)
      ...
      for (i in 1:length(mod$parameters$mean)){
        ...
        p1<-p1+stat_function(fun=adjusteddnorm, n=1000, args=list(mm=mod$parameters$mean[i], sd=mod$sd[i], prop=mod$parameters$pro[i]), aes(fill=!!colors[i], color=!!colors[i]), geom="area", alpha = .2)+
          ...
          xlab("Distance (bp)")+scale_color_discrete(guide=FALSE)+
...
mol_distance_distribution_20210401_test_filtering(rcmappath, xmap, qcmappath, startlabelid, endlabelid, nk=NULL, additional_noise_removal=FALSE)
