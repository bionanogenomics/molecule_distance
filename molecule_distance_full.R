...
#' @return .csv tables of distances between labels IDs (complete & trimmed data) & various plots (simple, auto-clustering, delta ref)


# load packages for clustering & visualization
library(mclust)
library(prabclus)
library(ggplot2)
                                 ...
                                 )
{
  ...
  # trimming - remove outliers beyond 3 s.d. from complete data, output data
  print(delta_mean<-mean(completeframe$delta))
  print(delta_sd<-sd(completeframe$delta))
  trimmeddata<-subset(completeframe, abs(completeframe$delta) < (delta_mean+(3*delta_sd)))

  write.csv(file = paste0(out_handle, "_trimmed_data.csv"), x = trimmeddata)
  

  # visualization
  waterfall_plotting_distance(simple_complete_output, paste0(out_handle,"_complete"))
  histogram_plot(simple_complete_output, out_handle)
  violin_plot(simple_complete_output, out_handle)
  summary_stats(simple_complete_output, out_handle)

  waterfall_plotting_distance(trimmeddata, paste0(out_handle,"_trimmed"))
  waterfall_plotting_delta(trimmeddata, paste0(out_handle,"_trimmed"))
  
  clustering(trimmeddata,
            startlabelid, 
            endlabelid, 
            out_handle,
            nk = NULL, 
